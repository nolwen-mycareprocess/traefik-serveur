...
class EAFrontend
{
    ...
    public function init()
    {
        ...
        add_shortcode('ea_simple_submit', array($this, 'ea_render_simple_submit_button')); // ✅ méthode appelée dans la classe
    }
    ...
    public function init_scripts()
    {
        ...
        wp_register_script('ea-momentjs', EA_PLUGIN_URL . 'js/libs/moment.min.js', array(), EASY_APPOINTMENTS_VERSION, true);
        wp_register_script('ea-validator', EA_PLUGIN_URL . 'js/libs/jquery.validate.min.js', array('jquery'), EASY_APPOINTMENTS_VERSION, true);
        wp_register_script('ea-masked', EA_PLUGIN_URL . 'js/libs/jquery.inputmask.min.js', array('jquery'), EASY_APPOINTMENTS_VERSION, true);
        wp_register_script('ea-datepicker-localization', EA_PLUGIN_URL . 'js/libs/jquery-ui-i18n.min.js', array('jquery', 'jquery-ui-datepicker'), EASY_APPOINTMENTS_VERSION, true);
        wp_register_script('ea-front-end', EA_PLUGIN_URL . 'js/frontend.js', array('jquery', 'jquery-ui-datepicker', 'ea-datepicker-localization', 'ea-momentjs', 'ea-masked'), EASY_APPOINTMENTS_VERSION, true);
        wp_register_script('ea-bootstrap', EA_PLUGIN_URL . 'components/bootstrap/js/bootstrap.js', array(), EASY_APPOINTMENTS_VERSION, true);
        wp_register_script('ea-front-bootstrap', EA_PLUGIN_URL . 'js/frontend-bootstrap.js', array('jquery', 'jquery-ui-datepicker', 'ea-datepicker-localization', 'ea-momentjs', 'ea-masked'), EASY_APPOINTMENTS_VERSION, true);
        wp_register_script('ea-google-recaptcha', 'https://www.google.com/recaptcha/api.js', array(), EASY_APPOINTMENTS_VERSION, true);
        ...
        wp_register_style('ea-jqueryui-style', EA_PLUGIN_URL . 'css/jquery-ui.css');
        wp_register_style('ea-bootstrap', EA_PLUGIN_URL . 'components/bootstrap/ea-css/bootstrap.css', array(), EASY_APPOINTMENTS_VERSION);
        wp_register_style('ea-bootstrap-select', EA_PLUGIN_URL . 'components/bootstrap-select/css/bootstrap-select.css', array(), EASY_APPOINTMENTS_VERSION);
        wp_register_style('ea-frontend-style', EA_PLUGIN_URL . 'css/eafront.css', array(), EASY_APPOINTMENTS_VERSION);
        wp_register_style('ea-frontend-bootstrap', EA_PLUGIN_URL . 'css/eafront-bootstrap.css', array(), EASY_APPOINTMENTS_VERSION);
        wp_register_style('ea-admin-awesome-css', EA_PLUGIN_URL . 'css/font-awesome.css', array(), EASY_APPOINTMENTS_VERSION);
        wp_register_style('ea-admin-fonts-css', EA_PLUGIN_URL . 'css/fonts.css', array(), EASY_APPOINTMENTS_VERSION);
    }

    public function ea_render_simple_submit_button($atts)
    {
        $atts = shortcode_atts(array(
            'location' => '',
            'service' => '',
            'worker' => '',
            'date' => '',  // valeur par défaut, modifiable par l'utilisateur
            'time' => '',  // idem
            'name' => '',
            'email' => '',
            'phone' => '',
        ...
        if (empty($atts['location']) || empty($atts['service']) || empty($atts['worker'])) {
            return '<p style="color:red;">⚠️ Paramètres manquants pour créer un rendez-vous.</p>';
        }

        ob_start();
?>
        <form class="ea-simple-form" method="post" action="<?php echo esc_url(admin_url('admin-ajax.php')); ?>">
            <input type="hidden" name="action" value="ea_ajax_save_appointment">
            <input type="hidden" name="location" value="<?php echo esc_attr($atts['location']); ?>">
            <input type="hidden" name="service" value="<?php echo esc_attr($atts['service']); ?>">
            <input type="hidden" name="worker" value="<?php echo esc_attr($atts['worker']); ?>">
            <input type="hidden" name="name" value="<?php echo esc_attr($atts['name']); ?>">
            ...
            <input type="hidden" name="note" value="<?php echo esc_attr($atts['note']); ?>">
            <input type="hidden" name="nonce" value="<?php echo esc_attr(wp_create_nonce('ea-bootstrap-form')); ?>">

            <label for="ea-date"><?php _e('Choisissez une date', 'easy-appointments'); ?> :</label>
            <input type="date" id="ea-date" name="date" value="<?php echo esc_attr($atts['date']); ?>" required>

            <br><br>

            <label for="ea-time"><?php _e('Choisissez une heure', 'easy-appointments'); ?> :</label>
            <input type="time" id="ea-time" name="time" value="<?php echo esc_attr($atts['time']); ?>" required>

            <br><br>

            <button type="submit" class="ea-btn"><?php _e('Réserver maintenant', 'easy-appointments'); ?></button>
        </form>
    <?php
        return ob_get_clean();
    }
